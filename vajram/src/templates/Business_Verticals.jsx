...
export default function Business_Verticals() {
  ...
  const toggleAccordion = (index) => {
    setActiveIndex(index); // Always set the clicked index as active, without toggling off
  };
      ...
      <style>{`
        ...
        @media (min-width: 768px) {
          ...
          .custom-height:hover p.text {
            opacity: 1; /* Fully visible in desktop view */
          }
        }
        ...
        <div className="container z-10 mx-auto absolute inset-0 ">
          ...
          <div className='flex md:flex-row flex-col items-center h-full absolute top-20'>
            {content.map((item, index) => (
              <div
                key={index}
                className={`flex flex-col custom-height w-full p-5 py-8 md:p-10 ${
                  activeIndex === index ? 'open' : ''
                } ${index !== content.length - 1 ? 'md:border-r-[1px] border-[#222528] md:border-b-0 border-b-2' : ''}`}
              >
